...
var arrowClicks=0;nextButton.on('click',function(){
    $('.ar-work').removeClass('active');gsap.to('.ar-work',{x:"-100%"}
    )})
...
function slideChecko(){currentSlide=$('.swiper-slide-active');
...
ss2ImagesSlider.on('slideChange',function(){
    ...
    let slideOut=gsap.timeline({
        yoyo:true,onComplete:function(){$(activeDets).removeClass('active')
...
ss2ImagesSlider.on('slideChangeTransitionEnd',function(){
    slideChecko();
    $('.ss2-button a').attr('href',activeURL);
    $('.ss2-curr').text('0'+activeIndex);let slideIn=gsap.timeline({
        yoyo:true,onStart:function(){
        $(activeDets).addClass('active');$(backText).addClass('active');
    }})
slideIn.fromTo(titleChar,{
    y:'100%'},{y:'0%',ease:'power2.out',stagger:0.01},0)
...
ss2ImagesSlider.on('transitionEnd',function(){
    let activeSlid=$('.swiper-slide-active'),activeIn=activeSlid.data('slide'),totSlid=$('.ss2-images .swiper-slide').length,lastSlid='.slide_'+totSlid;
    if(activeIn===lastSlid){this.mousewheel.disable();
    }})
...
function initShowcases(){
    ...
    if(showcaseCheck.length>0){
      if(showcaseCheck.hasClass('showcase-list')){aliothShowcaseList();
   ...
   function showcaseOpenings(){
       ...
       if(showcaseCheck.length>0){
        if(showcaseCheck.hasClass('showcase-list')){
            ...
            scWelcome.fromTo('.sl-project-title',2,{y:'110%'},{y:'0%',ease:'power2.out',stagger:0.1,},0)
...
scWelcome.fromTo('.showcase-footer',.6,{opacity:0},{opacity:1,},2)
    ...
    var wallOpen=gsap.timeline({once:true,delay:0,onStart:function(){
        $('body').addClass('loading')},onComplete:function(){$('body').removeClass('loading')}})
...
if(!mobileQuery.matches){aliothParallaxScroll();
}
enableScroll()
...
window.scrollTo(0,0);initShowcases();siteHeaderSet();

if($('.section.fullscreen').length){$('#footer').hide()}
else{$('#footer').show()
}
initShortcodes();
initPages();
